...
from utility import clear_variable
...
# help="scale images to this size before cropping to 256x256")，原 286
scale_size = 256
...
EPS = 1e-12               # minimum，avoid gradient descent to equal 0
image_size = 256          # image size

# 命名元组，用于存放加載的數據集合和創建好的模型
...
# generator's convolution，kernel is (4, 4), stride is 2，output that image is a half input's image width
...
# load example, the step is:
#   load file, decode, normalization, split data into input and target, change image value from [0, 1] to [1, -1], reshape
def load_examples(input_dir):
    ...
    # Ordering by number if first char of file name is number, or using alphabet
    ...
    with tf.name_scope("load_images"):
        ...
        raw_input = tf.image.convert_image_dtype(
            raw_input, dtype=tf.float32)  # 归一化处理
...
def transform(image):
    ...
    r = tf.image.resize_images(
        r, [scale_size, scale_size], method=tf.image.ResizeMethod.AREA)

    offset = tf.cast(tf.floor(tf.random_uniform(
        [2], 0, scale_size - image_size + 1, seed=seed)), dtype=tf.int32)
    if scale_size > image_size:
        r = tf.image.crop_to_bounding_box(
            r, offset[0], offset[1], image_size, image_size)
...
# 创建 Pix2Pix 模型，inputs 和 targets 形状为：[batch_size, height, width, channels]
...
def convert(image):
    if aspect_ratio != 1.0:
        ...
        size = [image_size, int(round(image_size * aspect_ratio))]
...
# 主函数
def train(train_input_dir, train_output_dir):
    # 设置随机数种子的值
    global seed
    if seed is None:
...
def test(input_dir, output_dir, model_path):
    seed = random.randint(0, 2 ** 31 - 1)
    ...
    # inputs and targets is [batch_size, height, width, channels]
    model = create_model(examples.inputs, examples.targets)
    print ("create model successful!")
    ...
    coord = tf.train.Coordinator()
    tf.train.start_queue_runners(coord=coord)
    for _ in range(examples.count):
        results = sess.run(display_fetches)
        filesets = save_images(output_dir,results)

        for _, _file in enumerate(filesets):
            print("evaluated image", _file["name"])

        index_path = append_index(output_dir, filesets)
